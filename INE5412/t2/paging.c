...
/**
 * @brief Swaps a page in out disk.
 * 
...
PRIVATE struct
{
	...
	unsigned tlu; 		/** Time of last usage.   */
	int age; 			/** Page age.             */
...
{
	int tbr; 			/* Page to be removed. */
	int oldest; 		/* Oldest page in process table. */
	int clean_pg;   	/* First clean page found.  */
	int sched;			/* Page whose write has been scheduled. 	*/

	int t;				/* WSClock minimum age. */
	int i;      		/* Local WSClock hand.  */

	struct pte *pg; 	/* Get page info.  */
	...
	sched = -1;
	t = 15;
	i = curr_proc->clock_hand;

	/* Find frame suitable for allocation. */
	for (int k = 0; k < NR_FRAMES; k++) 
	{	
		i = (i+1) % NR_FRAMES;
		...
		/* WSClock algorithm: local replacement policy. */
		if (frames[i].owner == curr_proc->pid) {
			...
			/* Finds info for current page.  */
			pg = getpte(curr_proc, frames[i].addr & PAGE_MASK);

			/* Saves first clean page.  */
			if (clean_pg < 0 && pg->dirty == 0)
				clean_pg = i;

			/* If R = 1.  */
			if (pg->accessed == 1) {
				pg->accessed = 0;  // sets R = 0
				frames[i].tlu = curr_proc->utime;  // updates TLU
				continue;  // advances pointer
			}
			
			/* Calculate page age.  */
			int pg_age = curr_proc->utime - frames[i].tlu;

			/* If R = 0 and age <= t.  */
			if (pg_age <= t) {
				if (pg_age > oldest) {
					oldest = i;  // saves page with largest age
				}
				continue;  // advances pointer
			}

			/* If R = 0, age > t and M = 0.  */
			if (pg->dirty == 0) {
				tbr = i;  // found a suitable page
				curr_proc->clock_hand = i; // save clock hand
				break;
			}	

			/* 	If R = 0, age > t and M = 1, 
				schedules exactly one write.  */
			if (sched < 0)
				sched = i;
		}
	}

	/* No page was selected for replacement.  */
	if (tbr < 0) {

		/* If a page was scheduled for write, swap it out.  */
		if (sched >= 0) {
			i = sched;
			curr_proc->clock_hand = sched;
			if (swap_out(curr_proc, frames[i].addr)) 
					return (-1);
		
		/* Else, if a clean page was found.  */
		} else if (clean_pg >= 0) {
			i = clean_pg;
			curr_proc->clock_hand = clean_pg;
		
		/* Finally, if no clean page was found.  */
		} else {
			curr_proc->clock_hand = i;
			i = oldest;
			if (swap_out(curr_proc, frames[i].addr)) 
				return (-1);
		}
